...
def build_admin_rest_url(shop: str, path: str) -> str:
    ...
    return f"https://{shop}/admin/api/{SHOPIFY_API_VERSION}/{clean_path}"
...
async def force_refresh_access_token(shop: str) -> str:
    """
    ...
    refresh_token = store.get("refresh_token")
    if not refresh_token:
        raise Exception(f"No refresh token available for {shop}")
    ...
    update_store_tokens(
        ...
        new_access_token,
        new_refresh_token,
        scopes=new_scopes or store.get("scopes", ""),
    )

    log_token_event(
        shop,
        "token_force_refresh",
        old_access=store.get("access_token"),
        new_access=new_access_token,
        old_refresh=refresh_token,
        new_refresh=new_refresh_token,
        old_scopes=store.get("scopes"),
        new_scopes=new_scopes or store.get("scopes", ""),
    )
